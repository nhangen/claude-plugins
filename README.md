...
| claude-mem-graph | Causal tracing layer over claude-mem. Extracts informed_by edges from observation narratives to trace what led to what. Cross-project search, session arc tracking, file history, playbook lineage. 6 MCP tools, 10 edge types. | `claude plugin install claude-mem-graph@nhangen-tools` |
