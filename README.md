...
| manus-dispatch | Dispatch long-running research tasks to the Manus AI agent platform from Claude Code via slash command. On-demand polling (no orphaned background processes), optional Obsidian filing, explicit-invocation cost gating. Manus does the autonomous research; Claude orchestrates. | `claude plugin install manus-dispatch@nhangen-tools` |
